    ...
    <div v-for="order in orderList" :key="order.pid" class="row mt-2 pt-3 justify-content-around">
      ...
      <div class="col-md-5 px-3">
        <div class="card-block px-3">
          <h6 class="card-title">
            <router-link v-bind:to="'/order/'+order.id">N º de pedido : {{order.id}}</router-link>
          </h6>
          <p class="mb-0">{{order.totalItems}} Artículo<span v-if="order.totalItems > 1">s</span></p>
          <p id="item-price" class="mb-0 font-weight-bold">Precio Total : $ {{order.totalCost}}</p>
          <p id="item-total-price">Ordenado el : {{order.orderdate}}</p>
        </div>
      </div>
      <div class="col-2"></div>
